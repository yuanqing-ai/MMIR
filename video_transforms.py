...
class GroupNormalize(object):
    def __init__(self, mean, std, threed_data=False,is_flow=False):
...
class Stack(object):
    ...
    def __call__(self, img_group):
        if img_group[0].mode == 'L':
            img = np.stack([x for x in img_group], axis=0)
            img = np.split(img,img.shape[0]//2,axis=0)
            return np.stack([np.stack([x[0],x[1]],axis=-1) for x in img], axis=0)
