...
def random_clip(record,video_frames, sampling_rate, frames_per_clip, fixed_offset=False,synchronise=False):
    """
    Args:
    ...
    num_sample_frame = frames_per_clip
    half_sample_frame = int(frames_per_clip/2)
    segment_images = []
    segment_flow = []
    step = 2
    segment_start = int(record.start_frame) + (step*half_sample_frame)
    segment_end = int(record.end_frame)+1 - (step*half_sample_frame)
    ...
    if segment_start >= segment_end:
        ...
        segment_end = int(record.end_frame)
    ...
    frame_idx_rgb = [int(no) for no in range(center_frame_rgb - (step*half_sample_frame),center_frame_rgb + (step*half_sample_frame),step)]
    frame_idx_flow = [int(no/2) for no in range(center_frame_flow - (step*half_sample_frame),center_frame_flow + (step*half_sample_frame),step)]

    return frame_idx_rgb, frame_idx_flow

def random_clip_val(record,video_frames, sampling_rate, frames_per_clip, fixed_offset=False,synchronise=False):
    """
    Args:
        video_frames (int): total frame number of a video
        sampling_rate (int): sampling rate for clip, pick one every k frames
    ...
    for center_frame in np.linspace(segment_start,segment_end,7,dtype=np.int32)[1:-1]:
        seg_f = []
        seg_i = []
        seg_f = [int(no/2) for no in range(center_frame - (step*half_sample_frame),center_frame + (step*half_sample_frame),step)]
        seg_i = [no for no in range(center_frame - (step*half_sample_frame),center_frame + (step*half_sample_frame),step)]
        frame_idx_rgb.append(seg_i)
        frame_idx_flow.append(seg_f)
...
class VideoDataSet(data.Dataset):
    ...
    def __init__(self,dataset_path, root_path_rgb, root_path_flow, num_groups=16, frames_per_group=2, sample_offset=0, num_clips=1,
                 modality='rgb', dense_sampling=True, fixed_offset=True,
                 image_tmpl='{:10d}.jpg', transform_rgb=None,transform_flow=None, is_train=True, test_mode=False, seperator=' ',
                 filter_video=0, num_classes=8):
        """
        ...
        self.root_path_rgb = root_path_rgb
    ...
    def _image_path(self, directory, idx):
        ...
        return os.path.join(self.root_path, directory,"frame_"+str(idx).zfill(10)+'.jpg')
    ...
    def _load_image(self, directory, idx,modality):
        ...
        num_try = 0
        if modality == 'rgb':
            image_path_file = os.path.join(self.root_path_rgb, directory,"frame_"+str(idx).zfill(10)+'.jpg')
        else:
            flow_x_name = os.path.join(self.root_path_flow, directory,'u',"frame_"+str(idx).zfill(10)+'.jpg')
            flow_y_name = os.path.join(self.root_path_flow, directory,'v',"frame_"+str(idx).zfill(10)+'.jpg')
        img = None
        while num_try < 10:
            try:
                if modality == 'rgb':
    ...
    def _parse_list(self):
        ...
        if self.is_train:
            f = open(self.dataset_path+'_train.pkl','rb')
        else:
            f = open(self.dataset_path+'_test.pkl','rb')
        ...
        for _, line in df.iterrows():
            elements = [line['participant_id']+"/"+line['video_id'], line['start_frame'], line['stop_frame'],line['verb_class']]
            #print(line)
    ...
    def _sample_indices(self, record):
        ...
        """
        max_frame_idx = max(1, record.num_frames)
        if self.dense_sampling:
            frame_idx_rgb,frame_idx_flow = np.asarray(random_clip(record,max_frame_idx, self.sample_freq, self.num_frames))
        # else:
        ...
        return frame_idx_rgb,frame_idx_flow
    def _sample_indices_val(self, record):
        """
        Used for training.
        Args:
            - record (VideoRecord):
        ...
        max_frame_idx = max(1, record.num_frames)
        if self.dense_sampling:
            frame_idx_rgb,frame_idx_flow = np.asarray(random_clip_val(record, max_frame_idx, self.sample_freq, self.num_frames))
        # else:
        #     total_frames = self.num_groups * self.frames_per_group
        #     ave_frames_per_group = max_frame_idx // self.num_groups
        #     if ave_frames_per_group >= self.frames_per_group:
    ...
    def __getitem__(self, index):
        ...
        if self.is_train:
            indices_rgb,indices_flow = self._sample_indices(record)
            images_rgb = []
            images_flow = []
            for seg_ind in indices_rgb:
                seg_imgs_rgb = self._load_image(record.path, seg_ind,'rgb')

                images_rgb.extend(seg_imgs_rgb)

            for seg_ind in indices_flow:
                seg_imgs_flow = self._load_image(record.path, seg_ind,'flow')
                images_flow.extend(seg_imgs_flow)


            images_rgb = self.transform_rgb(images_rgb)
            images_flow = self.transform_flow(images_flow)
            ...
            indices_rgb,indices_flow = self._sample_indices_val(record)
            images_rgb = []
            images_flow = []
            for seg_ind in indices_rgb:
                for seg in seg_ind:
                    seg_imgs_rgb = self._load_image(record.path, seg,'rgb')
                    images_rgb.extend(seg_imgs_rgb)

            for seg_ind in indices_flow:
                for seg in seg_ind:
                    seg_imgs_flow = self._load_image(record.path, seg,'flow')
                    images_flow.extend(seg_imgs_flow)
            images_rgb = self.transform_rgb(images_rgb)
            images_flow = self.transform_flow(images_flow)
        ...
        return images_rgb,images_flow, label
