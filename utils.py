...
def get_augmentor(is_train, image_size, mean=None,
    ...
    scale_range = [256, 456] if scale_range is None else scale_range
    augments = []
    ...
    augments += [
        ...
        ToTorchFormatTensor(),
        GroupNormalize(mean=mean, std=std, threed_data=threed_data,is_flow=is_flow)
    ]
...
def build_dataflow(dataset, is_train, batch_size, workers=2, is_distributed=False):
